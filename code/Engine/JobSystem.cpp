...
#pragma optimize( "", on )
	...
	static inline SignalHandle allocateSignal()
	{
		SE_FATAL(!g_system->m_free_queue.empty());
	...
	{
		SE_FATAL((handle & HANDLE_ID_MASK) < 4096);
	...
	static inline bool isSignalZero(SignalHandle handle, bool lock)
	{
	...
	static inline void runInternal(void* data
		, void (*task)(void*)
		, SignalHandle precondition
		, bool do_lock
	...
	void enableBackupWorker(bool enable)
	{
		...
		ASSERT(enable);
		WorkerTask* task = SE_NEW(g_system->m_allocator, WorkerTask)(*g_system, 0xff);
		if (task->create("Backup worker", false)) {
			...
			SE_LOG_ERROR("Job system backup worker failed to initialize.");
			SE_DELETE(g_system->m_allocator, task);
		}
	}
...
#endif
	{
		...
		while (!worker->m_finished) {
			...
			if (fiber) {
				...
				g_system->m_sync.enter();
				SE_FATAL(!this_fiber->current_job.task);
	...
	bool init(u8 workers_count, IAllocator& allocator)
	{
		...
		for (int i = 0; i < count; ++i) {
			WorkerTask* task = SE_NEW(allocator, WorkerTask)(*g_system, i < 64 ? u64(1) << i : 0);
			if (task->create("Worker", false)) {
				...
				SE_LOG_ERROR("Job system worker failed to initialize.");
				SE_DELETE(allocator, task);
			}
		}
