...
struct InputSystemImpl final : InputSystem
{
	...
	{
		m_mouse_device = SE_NEW(m_allocator, MouseDevice);
		m_mouse_device->type = Device::MOUSE;
		m_keyboard_device = SE_NEW(m_allocator, KeyboardDevice);
...
#define REGISTER_KEYCODE(KEYCODE) \
		LuaWrapper::createSystemVariable(state, "SEAPI", "INPUT_KEYCODE_" #KEYCODE, (int)OS::Keycode::KEYCODE);
