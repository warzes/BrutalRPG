...
namespace { struct NewPlaceholder {}; }
...
#define SE_NEW(allocator, ...) new (NewPlaceholder(), (allocator).allocate_aligned(sizeof(__VA_ARGS__), alignof(__VA_ARGS__))) __VA_ARGS__
#define SE_DELETE(allocator, var) (allocator).deleteObject(var);
...
struct UniquePtr {
	...
	template <typename... Args> static UniquePtr<T> create(IAllocator& allocator, Args&&... args) {
		return UniquePtr<T>(SE_NEW(allocator, T) (static_cast<Args&&>(args)...), &allocator);
	}
