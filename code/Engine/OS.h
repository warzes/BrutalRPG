...
namespace OS
{
	...
	struct InputFile final : IInputStream
	{
	public:
		InputFile();
		~InputFile();
	...
	struct OutputFile final : IOutputStream
	{
	public:
		OutputFile();
		~OutputFile();
	...
	void getCurrentDirectory(Span<char> path);
	...
	bool getOpenDirectory(Span<char> out, const char* starting_dir);
	...
	void clipCursor(int screen_x, int screen_y, int w, int h);
	void unclipCursor();
	...
	void getExecutablePath(Span<char> path);
